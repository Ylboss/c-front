    ...
    send() {
      ...
      if (!this.text) {
        ...
        if (typeof (WebSocket) == "undefined") {
          ...
          let message = { from: this.user.username, to: this.chatUser, text: this.text ,timestamp:this.timestamp }
    ...
    init() {
      ...
      if (typeof (WebSocket) == "undefined") {
        ...
        socket.onmessage = function (msg) {
          ...
          try {
            let data = JSON.parse(msg.data); // 解析JSON数据

            // 使用Object.prototype.hasOwnProperty来检查属性
            if (Object.prototype.hasOwnProperty.call(data, 'users')) {
              // 如果数据包含users属性，则是用户列表更新消息
              Message.success("在线用户更新：" + JSON.stringify(data.users));
            } else if (Object.prototype.hasOwnProperty.call(data, 'from') && Object.prototype.hasOwnProperty.call(data, 'text')) {
              // 如果数据包含from和text属性，则是聊天消息
              Message.info({
                message:"收到消息："+data.from+"说："+data.text,
                showClose: true,
              })
            } else if (Object.prototype.hasOwnProperty.call(data, 'error')) {
              // 如果数据包含error属性，则是错误消息
              Message.error("发生错误：" + data.error);
            } else {
              // 其他未知类型的消息
              Message.warning("收到未知类型消息：" + JSON.stringify(data));
            }
          } catch (e) {
            console.error("解析消息时出错：", e);
            Message.error("收到的消息格式不正确：" + msg.data);
          }
