<template>
    <div>
        ...
        <div style="margin: 10px 0">
            <el-button type="primary" @click="handleAdd" v-if="user.role === 'ROLE_USER'||user.role === 'ROLE_TEACHER'">新增 <i
        ...
        <el-table :data="tableData" border stripe :header-cell-class-name="'headerBg'"
            ...
            <el-table-column label="操作" width="280" align="center">
                <template slot-scope="scope">
                    <el-button type="success" @click="handleEdit(scope.row)" v-if="user.role === 'ROLE_USER'">编辑 <i
                            class="el-icon-edit"></i></el-button>
                    <el-popconfirm class="ml-5" confirm-button-text='确定' cancel-button-text='我再想想' icon="el-icon-info"
                        icon-color="red" title="您确定删除吗？" @confirm="del(scope.row.id)">
                        <el-button type="danger" slot="reference" v-if="user.role === 'ROLE_USER'">删除 <i
    ...
    methods: {
        ...
        imgAdd(pos, $file) {
            ...
            axios({
                ...
                method: 'post',
                data: {formData,uploaderName:this.user.name,purpose:'其他用户上传文章'},
        ...
        load() {
            this.request.get("/article/page", {
                params: {
                    ...
                    name: this.name,
                    // userId: this.user.id // 传递登录用户的ID
                }
            }).then(res => {
                const filteredData = res.data.records.filter(item => item.userId === this.user.id);
                if (filteredData.length > 0) { // 判断是否有与登录用户相关的数据
                    this.tableData = filteredData;
                    this.total = res.data.total;
                } else {
                    this.tableData = []; // 清空数据，不显示非登录用户相关的数据
                    this.total = 0;
                }
            }).catch(error => {
